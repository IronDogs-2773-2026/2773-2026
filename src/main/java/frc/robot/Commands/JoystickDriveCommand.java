...
public class JoystickDriveCommand extends Command {
  ...
  private void doDrive() {
    sensitivity = MathUtil.clamp(joystick.getThrottle(), 0.05, 1);
    ...
    double rotSpeed = (MathUtil.applyDeadband(ZAxis, Constants.ControllerDeadzone)) * sensitivity
        * Constants.MaxRotationSpeed;

    pid.setSetpoint(setDistance);

    double driveSpeed = pid.calculate((driveSub.averageDistanceEncoder() - oldDist) * 11.24)
        * Constants.MaxDriveSpeed * sensitivity;

    if (Math.abs(XAxis) < Constants.ControllerDeadzone && Math.abs(YAxis) < Constants.ControllerDeadzone
      ...
      driveSub.directionalDrive(driveSpeed, deltaAngle, rotSpeed);
    }
    oldDist = driveSub.averageDistanceEncoder();
    oldRot = odomSub.getGyroAngle();
  }

  private void doButtons() {
    if (joystick.getRawButton(11) && joystick.getRawButton(2)) {
      odomSub.resetGyro();
    }
  }
